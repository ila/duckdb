...
unique_ptr<CteNode> LogicalPlanToSql::CreateCteNode(
    unique_ptr<LogicalOperator> &subplan, const vector<size_t>& children_indices
) {
	...
	case LogicalOperatorType::LOGICAL_UNION: {
		...
		for (size_t i = 0; i < lhs_bindings.size(); ++i) {
			...
			unique_ptr<ColStruct> new_col_struct = make_uniq<ColStruct>(
			    table_index, lhs_col_struct->column_name, lhs_col_struct->alias
			);
			cte_column_names.push_back(new_col_struct->ToUniqueColumnName());
