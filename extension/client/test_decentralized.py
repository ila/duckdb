...
def send_to_postgres(i, run):
    ...
    try:
        ...
        with sqlite3.connect(db_path) as sqlite_conn:
            ...
            rows = sqlite_conn.execute("SELECT * FROM daily_steps_user LIMIT 1").fetchall()
        ...
        enriched = []
        for row in rows:
            nickname, city, club, date, steps = row
            client_id = int(nickname.split("_")[1])
            enriched.append((
                nickname, city, club, date, steps,
        ...
        try:
            with psycopg2.connect(params.SOURCE_POSTGRES_DSN) as pg_conn:
                with pg_conn.cursor() as cur:
                    cur.executemany(
                        """
                        INSERT INTO sidra_centralized_view_daily_steps_user (
                            nickname, city, club, date, total_steps,
...
def update_window():
    try:
        ...
        with socket.create_connection((server_addr, server_port), timeout=10) as sock:
            ...
            view = "sidra_centralized_view_daily_steps_user"
            view_len = len(view)
...
def main():
    ...
    max_activity_metrics = params.MAX_RUNS
    if params.UPDATE_WINDOW_EVERY_REFRESH:
        max_activity_metrics *= params.NUM_CHUNKS

    while run < max_activity_metrics:
