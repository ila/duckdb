...
create or replace table mv_daily_runs_city as select city, date, sum(steps) as total_steps, rdda_window
from rdda_client.rdda_centralized_view_runs
group by city, date, rdda_window;

create or replace table rdda_client.d_rdda_centralized_view_runs as select *, true as _duckdb_ivm_multiplicity, now() as _duckdb_ivm_timestamp
from rdda_client.rdda_centralized_view_runs limit 0;

create or replace table d_mv_daily_runs_city as select *, true as _duckdb_ivm_multiplicity from mv_daily_runs_city limit 0;

create unique index mv_daily_runs_city_ivm_index on mv_daily_runs_city(city, date, rdda_window);